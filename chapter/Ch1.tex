...
命题也有\textbf{命题常量}(propositional constant)
\index{propositional constant\kong 命题常量}
...
我们能否量化谓词变量？当然可以，
事实上在以后的学习中，我们会不时地遇到这种情况。
但在本章我们暂时不讨论，这已经超出了一阶逻辑的范围。
...
\index{atomic formula\kong 原子公式}是指：
...
\index{well-formed formula\kong （一阶逻辑）合成公式}
是指如下符号串：
...
使之允许出现更多种类的连接词、量词。这里的“合成公式”的全称为
\textbf{一阶谓词公式}，本讲义中我们简称为\textbf{公式}。
...
研究一阶谓词公式的逻辑学分支称之为\textbf{一阶谓词逻辑}，
简称\textbf{一阶逻辑}（first-order logic，简记为FOL）。
\index{first-order logic\kong 一阶逻辑}
一阶逻辑是哲学家、逻辑学家、语言学家、
理论计算机学家以及数学家的共同语言。 
本讲义将介绍其基本原理。一阶逻辑是数学基础中很重要的一部分，
它是公理系统的标准形式逻辑。许多常见的公理系统，
如算术的一阶Peano（皮亚诺）公理和
包含Zermelo–Fraenkel（策梅洛-弗兰克尔）
集合论的公理化集合论等，都可以形式化成一阶理论。
当然一阶逻辑也有其局限性，我们在后文会简单提一下。
...
\index{bound occurrence\kong （个体变量）约束出现}

（2）否则，称$x$在公式$A$中\textbf{自由出现}（free occurrence）。
\index{free occurrence\kong （个体变量）自由出现}
\label{自由出现与约束出现的定义}
\end{definition}
...
\index{interpretation\kong （一阶逻辑公式的）解释}
是指以下三部分：

（1）指定一个非空的论域$D$；

（2）给公式中的每个函数符号、
谓词符号指定一个相应的（定义在$D$上的）函数、谓词；

（3）给公式中的每个自由出现的个体变量指定一个
（$D$当中的）具体的个体。
\end{definition}
...
例如，取论域为全体实数，
将$F(x,y)$解释为“$x>y$”，$x,y$分别解释为2、3，则得到命题
“$2>3$或者$\neg(2>3)$”，这是真命题。
也可以取论域为所有人，并把$F(x,y)$解释成“$x$是$y$的父亲”，并且
...
\begin{proof}[Solution]
对于（1），我们考虑此公式的解释：
取论域为全体实数，$y=0$，$F(x,y)$为“$x>y$”，$G(y)$为“$y$是奇数”——
...
\begin{prob}试证明以下公式既不是永真式，也不是矛盾式：
